...
local somRegras = audio.loadStream( "audio/regrasSong.ogg" )
...
function scene:show( event )
   ...
   elseif ( phase == "did" ) then
       ...
       btnSom:addEventListener( "tap", pausaSom )
        ...
        btnProximo:addEventListener( "tap", telaRegras2 )
...
function configuraPainel()
    ...
    local painel = display.newImage( "graficos/componentes/painelRegras1.png", centroX - 90, centroY - 30)
        painel.xScale = .60
        painel.yScale = .55
        scene.view:insert(painel)
end

function telaRegras2()
    
    composer.removeScene( "regras1" )
    composer.gotoScene( "regras2", transicaoParaDireita )
end
