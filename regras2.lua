...
local btnProximo, btnVoltar, btnSom, btnSair
...
function scene:create( event )
   ...
   audio.play( somRegras , {loops = -1, channel = 3} )
...
function scene:show( event )
   ...
   elseif ( phase == "did" ) then
       ...
       btnSom:addEventListener( "tap", pausaSom )
        ...
        btnProximo:addEventListener( "tap", telaRegras3 )
        btnVoltar:addEventListener( "tap", telaRegras1 )
...
function configuraBotoes()
    ...
    btnProximo = display.newImage("graficos/botoes/btnProximo.png")
    btnProximo.xScale = .7
    btnProximo.yScale = .7
    btnProximo.x = largura - 80
    btnProximo.y = altura - 145
    scene.view:insert(btnProximo)
    
   btnVoltar = display.newImage("graficos/botoes/btnVoltar.png")
   btnVoltar.xScale = .7
   btnVoltar.yScale = .7
   btnVoltar.x = largura - 80
   btnVoltar.y = altura - 90
...
function configuraPainel()
    ...
    local painel = display.newImage( "graficos/componentes/painelRegras2.png", centroX - 90, centroY - 30)
        painel.xScale = .60
        painel.yScale = .55
        scene.view:insert(painel)
end

function telaRegras1()
    
    composer.removeScene( "regras2" )
    composer.gotoScene( "regras1", transicaoParaEsquerda )
end

function telaRegras3()
    
    composer.removeScene( "regras2" )
    composer.gotoScene( "regras3", transicaoParaDireita )
end
